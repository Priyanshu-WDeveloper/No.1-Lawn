...
export default function EmployeeManagementPage() {
  ...
  const { data: apiEmployees } = useGetEmployeesQuery(undefined);

  const allEmployees = useMemo(() => {
    const apiRows: EmployeeRow[] = (apiEmployees || []).map(
      (emp: any) => ({
        id: emp.id || emp._id,
        name: emp.name,
        email: emp.email,
    ...
    return [...sampleEmployees, ...apiRows];
  }, [apiEmployees]);
      ...
      cell: (row: EmployeeRow) => (
        <div className="flex flex-wrap gap-2">
          <ActionButton
            ...
            onClick={() =>
              navigate(ROUTES.EMPLOYEES_VIEW.replace(':id', row.id))
            }
          />
          <ActionButton
            intent="edit"
